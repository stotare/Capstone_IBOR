--**portfolio_holdings** is what portfolio managers check every morning. 'What do I own? How much is it worth? Am I making money?' This table answers all of that. It includes position weights - what % of the portfolio each holding represents. Which is critical for risk management.
...
with current_positions as (
    select * from {{ ref('silver_int_current_positions') }}
),
...
fx_rates as (
    ...
    where rate_date = (select max(rate_date) from {{ ref('bronze_fx_rates') }})
),
