--**portfolio_summary** is the executive view. The CIO doesn't want to see 50 individual positions - they want the big picture: total portfolio value, overall return %, asset allocation. One row per portfolio, all the key metrics.
...
with holdings as (
    select * from {{ ref('gold_Portfolio_holdings') }}
),
...
--Notice the materialization - both are TABLES, not views. Because portfolio managers and executives need FAST queries. They're refreshing dashboards, running reports. Tables give instant response times..
